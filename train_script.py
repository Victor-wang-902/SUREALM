...
class Handler:
        ...
        for epoch in tqdm.trange(self.args.epochs):
            ...
            if (epoch + 1) % self.args.save_epochs == 0:  # save model
                ...
                if self.args.device == torch.device("cuda") and torch.cuda.device_count() > 1:
                    self.args.model.module.save_pretrained(output_path, self.args.save_head)
                else:
                    self.args.model.save_pretrained(output_path, self.args.save_head)
    ...
    def evaluate(self):
        ...
        cur_perplexity = []
        cur_tokens = 0
        logging.info("start evaluating on validation")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_config', default=None, help="path to the data config relative to data_folder")
