...
from transformers.models.roberta import RobertaForCausalLM
import logging
...
class RetrievalGenerationModelRoberta(RobertaForCausalLM):
    def __init__(self, config, args=None, tokenizer=None):
        ...
        self.cross_attention = self.roberta.config.add_cross_attention
        self.is_retrieval = self.config.is_retrieval
        if args is not None:
    ...
    def load_embeddings(self, pre, post):
        self.roberta.load_embeddings(pre,post)

    def set_retrieval(self, bool=True, pre=None, post=None):
        if bool:
            self.roberta.is_retrieval(pre, post)
        else:
            self.roberta.disable_retrieval()
        self.is_retrieval = bool

    def post_post_init(self, args=None, from_checkpoint=None, has_cross_attention=None, init_cross_attention_weights=None, not_cross_attention=None, not_retrieval=None, pre_path=None, post_path=None, freeze_backbone=None, max_length=None, max_context_length_per_k=None): # if the model is initialized from huggingface pretrained model without cross attention, call this function after from_pretrained
        if args is not None:
            from_checkpoint = args.from_checkpoint
            has_cross_attention = args.has_cross_attention
            init_cross_attention_weights = args.init_cross_attention_weights
            not_cross_attention = args.not_cross_attention
            not_retrieval = args.not_retrieval
            pre_path = args.pre_path
            post_path = args.post_path
            freeze_backbone = args.freeze_backbone
            max_length = args.max_length
            max_context_length_per_k = args.max_context_length_per_k
        if from_checkpoint is not None and has_cross_attention is not None and init_cross_attention_weights is not None:
            if not from_checkpoint:
                if self.roberta.config.add_cross_attention and not has_cross_attention: # initialize model specific parameters from original model with modified config
                    if init_cross_attention_weights:
                        self.roberta.init_cross_attention_weights()
        if not_cross_attention is not None:
            self.cross_attention = not not_cross_attention
        if not_retrieval is not None:
            self.set_retrieval(not not_retrieval, pre_path, post_path)
    ...
    # here encoder does not necessarily mean cross attention
    def forward(self, 
        ...
        # condition checking. Copied from model body
        if labels is not None:
            use_cache = False
        if self.is_retrieval:
            assert (encoder_hidden_keys is None and encoder_hidden_values is None) or (encoder_hidden_keys is not None and encoder_hidden_values is not None)
            assert encoder_indices is None or encoder_hidden_keys is None
            assert encoder_indices is not None or encoder_hidden_keys is not None
            assert encoder_hidden_states is None # regular seq2seq embedding does not apply here
        else:
            assert encoder_hidden_keys is None and encoder_hidden_values is None and encoder_indices is None
    
        if self.is_retrieval:
            seq_len = len(input_ids)
            if encoder_hidden_keys is not None:
                ctx_len = len(encoder_hidden_keys)
            elif encoder_indices is not None:
                ctx_len = len(encoder_indices)
            mask = encoder_attention_mask
            if not self.cross_attention:
                attention_mask = mask
                encoder_attention_mask = None
        '''
        print("head arguments")
        print("iid", input_ids)
        print("atnmsk", attention_mask)
        print("enhs", encoder_hidden_states)
        print("enhk", encoder_hidden_keys)
        print("enhv", encoder_hidden_values)
        print("eni", encoder_indices)
        print("enatnmsk", encoder_attention_mask)
        ...
        prediction_scores = self.lm_head(sentence_output)
    ...
    def save_pretrained(self, output_path, save_head=False, save_emb=False):
        ...
        if save_head:
            if not save_emb:
                try:
                    key, value = self.roberta.clear_embeddings()
                except:
                    key, value = None, None
...
class RetrievalGeneratorRoberta(RetrievalGenerationModelRoberta):
    def __init__(self, config, args=None, tokenizer=None):
        super().__init__(config, args=args, tokenizer=tokenizer)

    def load_components(self, knowledge_path=None, sbert_path=None):
    ...
    def load_knowledge(self, knowledge_path):
        ...
        #print(self.knowledge_pre.weight)
        _, stride, topk = os.path.basename(knowledge_path).split("_")
        stride = int(stride[0])
        topk = int(topk[-1])
        self.knowledge_stride = stride
        self.knowledge_topk = topk
        self.mask_gen = AttentionMaskGenerator(self.knowledge_topk)
    ...
    def prepare_inputs_for_generation(self, input_ids, **kwargs):
        ...
        num_real_tokens = torch.sum(input_ids != self.pad_token_id, dim=1)
        real_token_positions = torch.ne(input_ids, self.pad_token_id)
        #attention_mask = pad_sequence([torch.ones(i) for i in num_real_tokens], batch_first=True)
        attention_mask = torch.zeros_like(input_ids).type(torch.int32)
        attention_mask[input_ids != self.pad_token_id] = 1
        #new_input_ids = torch.zeros_like(attention_mask).long().to(self.device)
        #new_input_ids[attention_mask.bool()] = input_ids[real_token_positions]
        ...
        #print(self.knowledge_pre.weight)
        if not self.cached:
            cur_num_real_tokens = self.retrieval_stride + 1
            #cur_num_real_tokens = torch.tensor([self.retrieval_stride + 1] * num_real_tokens.shape[0]).to(self.device)
            #while not torch.all(cur_num_real_tokens > num_real_tokens):
            while not cur_num_real_tokens > total_len:

                #cur_num_real_tokens = min(cur_num_real_tokens, num_real_tokens)
                #input_ids_list = [input_ids[i,:cur_num_real_tokens[i]] for i in range(len(cur_num_real_tokens))]
                input_ids_list = input_ids[:, :cur_num_real_tokens]
                attention_msk = attention_mask[:, :cur_num_real_tokens]
                #print("startup phase", input_ids_list, attention_msk)
                #to_update = 
                #input_ids_list = [input_ids[i,:num_real_tokens[i] - ((num_real_tokens[i] - 1) % self.retrieval_stride)] for i in range(len(num_real_tokens))]
                ...
                #pre_embs2 = encode({"input_ids": input_ids_list[0:, :num_real_tokens[0]].to(self.device), "attention_mask": attention_msk[0:,:num_real_tokens[0]].to(self.device)}, model=self.sbert)
                #print("HERE",pre_embs,pre_embs2)
                _, top_indices = self.index.search(pre_embs, self.retrieval_topk)
                #print(self.knowledge_pre(torch.tensor([0]).to(self.device)))
                #raise Exception
                ...
                for i, (pre, post) in enumerate(zip(new_pre_embs, new_post_embs)):
                    if to_update[i]:
                        self.encoder_cache[0][i] = torch.cat((self.encoder_cache[0][i], pre), dim=0)
                        self.encoder_cache[1][i] = torch.cat((self.encoder_cache[1][i], post), dim=0)
                        self.cached = True
                cur_num_real_tokens += self.retrieval_stride
            ...
            for i, (pre, post) in enumerate(zip(new_pre_embs, new_post_embs)):
                if to_update[i]:
                    self.encoder_cache[0][i] = torch.cat((self.encoder_cache[0][i], pre), dim=0)
                    self.encoder_cache[1][i] = torch.cat((self.encoder_cache[1][i], post), dim=0)
        #print("pad", self.knowledge_pre(torch.tensor([0]).to(self.device)))
        #raise Exception
        inputs = {"input_ids": input_ids, "attention_mask": attention_mask.to(self.device)}
        pre_list = pad_sequence(self.encoder_cache[0], batch_first=True)
        post_list = pad_sequence(self.encoder_cache[1], batch_first=True)

        #else:
        #    pre_list = self.knowledge_pre(torch.tensor([0]).to(self.device))
        #    post_list = self.knowledge_post(torch.tensor([0]).to(self.device))
        pre_list = pre_list[:,:self.config.max_context_length_per_k * self.retrieval_topk,:]
        post_list = post_list[:,:self.config.max_context_length_per_k * self.retrieval_topk,:]
        inputs["encoder_hidden_keys"] = pre_list
        inputs["encoder_hidden_values"] = post_list
        inputs["ngram"] = self.retrieval_stride
        #pad_end = time.time()
        #msk_start = time.time()
        ...
        #print(inputs["input_ids"], inputs["encoder_attention_mask"], inputs["attention_mask"], inputs["encoder_hidden_keys"].shape)
        #print(inputs["encoder_attention_mask"])
        return inputs
            
    def init_cache(self, input_ids):
        batch_size = input_ids.shape[0]
        self.encoder_cache = ([torch.empty(0, self.config.hidden_size).to(self.device)]*batch_size,[torch.empty(0, self.config.hidden_size).to(self.device)]*batch_size)
        self.cached = False
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        ```"""
        if retrieval_stride is None:
            self.retrieval_stride = self.knowledge_stride
        else:
            self.retrieval_stride = retrieval_stride
        if retrieval_topk is None:
            self.retrieval_topk = self.knowledge_topk
        else:
            self.retrieval_topk = knowledge_topk
        self.pad_token_id = pad_token_id
        self.init_cache(inputs)
        self.mask_gen = AttentionMaskGenerator(self.retrieval_topk)
