...
from wrappers import RetrievalGenerator
from wrappers_gpt2 import RetrievalGeneratorGPT2
import argparse
import torch

def inference(args, tokenizer, model, prompts="", device=torch.device("cpu"), **kwargs):
    outputs = []
    ...
    for prompt in prompts:
        ...
        outputs.extend(model.generate(inputs=input_ids, bos_token_id=tokenizer.bos_token_id, eos_token_id=tokenizer.eos_token_id, pad_token_id=tokenizer.pad_token_id, **kwargs))
...
def main():
    ...
    if args.interactive:
        while True:
            ...
            if prompt == "!quit":
                exit()
            print(inference(args, tok, model, prompt, device=args.device, retrieval_stride=args.stride, retrieval_topk=args.topk))
    else:
        with open(args.prompt_file, "r") as f:
            prompts = [line.strip() for line in f]
            print(inference(args, tok, model, prompts, device=args.device, bos_token_id=bos_token_id, eos_token_id=eos_token_id, pad_token_id=pad_token_id, retrieval_stride=args.stride, retrieval_topk=args.topk))
