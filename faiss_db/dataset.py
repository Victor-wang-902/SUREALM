...
from utils import pad_emb, AttentionMaskGenerator, NaiveAttentionMaskGenerator
import pickle
import numpy as np
import logging
import time
...
def split_test(data_folder, new_test_folder):
    with open(os.path.join(data_folder, "data_eval", "test", "logs.json"), "r") as f:
        entries = json.load(f)
    with open(os.path.join(data_folder, "data", "knowledge.json"), "r") as f:
        kb = json.load(f)
    ...
    for i, item in enumerate(entries):
        indomain = True
        try:
            for kb_entry in labels[i]["knowledge"]:
                try:
                    _ = kb[kb_entry["domain"]][kb_entry["entity_id"]]
                except KeyError:
                    indomain = False
                    break
        except KeyError:
        ...
        if indomain:
            ...
            outofdomain_test.append(item)
    os.makedirs(new_test_folder)
    with open(os.path.join(new_test_folder, "test_in.json"), "w") as f:
        json.dump(indomain_test, f, sort_keys=True, indent=4)
    with open(os.path.join(new_test_folder, "test_out.json"), "w") as f:
        json.dump(outofdomain_test, f, sort_keys=True, indent=4)

def split_test_domain(data_folder, new_test_folder):
    with open(os.path.join(data_folder, "data_eval", "test", "logs.json"), "r") as f:
        entries = json.load(f)
    with open(os.path.join(data_folder, "data", "knowledge.json"), "r") as f:
        kb = json.load(f)
    ...
    for i, item in enumerate(entries):
        attraction = False
        if "knowledge" in labels[i].keys():
            for kb_entry in labels[i]["knowledge"]:
                if kb_entry["domain"] == "attraction":
                    attraction = True
                    break
            ...
            other_test.append(item)
    os.makedirs(new_test_folder)
    with open(os.path.join(new_test_folder, "test_in.json"), "w") as f:
        json.dump(indomain_test, f, sort_keys=True, indent=4)
    with open(os.path.join(new_test_folder, "test_out.json"), "w") as f:
        json.dump(outofdomain_test, f, sort_keys=True, indent=4)
    with open(os.path.join(new_test_folder, "test_oth.json"), "w") as f:
        json.dump(other_test, f, sort_keys=True, indent=4)
...
class FastRetrievalDataset(Dataset):
    def __init__(self, data, args=None, val=False, data_folder="fast_processed_data", external_embedding=False, device=torch.device("cpu"), test=False):
        if args is not None:
            ...
            if not test:
                if not args.eval_only:
                    ...
                    if external_embedding:
                        ...
                        self.ind_path = os.path.join(data_folder, data["name"], "test_fast_indices.pkl")
            else:
                self.text_path = os.path.join(data_folder, data["name"], "data_test.txt")
                if external_embedding:
                    self.pre_path = os.path.join(data_folder, data["name"], "test_fast_prefix_embeddings.pkl")
...
class FastCollator:
    def __init__(self, data, args=None, max_length=128, max_context_length_per_k=128, tokenizer=AutoTokenizer.from_pretrained("bert-base-uncased"), concat_self=True):
        if args is not None:
    ...
    def __call__(self, batch):
        ...
        #raise Exception()
        #print("encodermask", inputs["encoder_attention_mask"].shape, inputs["encoder_attention_mask"])
        #print("encoderstates", inputs["encoder_hidden_keys"].shape)
        #print("hiddenstates", inputs["input_ids"].shape, inputs["input_ids"])
        #raise Exception
        return inputs
    ...
    def __init__(self, data, args=None, val=False, data_folder="processed_data", topk=3, external_embedding=False, device=torch.device("cpu")):
        ...
        if self.val:
            if not args.eval_only:
                ...
                self.mapping_path = os.path.join(data_folder, data["name"], "test_embedding_mappings.pkl")
                self.val_prefix_path = os.path.join(data_folder, data["name"], "test_prefix_embeddings.pt")
...
class TextDataset(Dataset):
    def __init__(self, data, args=None, val=False, data_folder=None, test=False):
        if args is not None:
            data_folder = args.data_folder
        if val:
            if not test:
...
def load_dataset(data, args, val=False, val_exc=False, test=False):
...
if __name__ == "__main__":
    extract_dstc("test_domain_new/test_in.json", "test_domain_new/test_in.txt")
    extract_dstc("test_domain_new/test_out.json", "test_domain_new/test_out.txt")
