...
def skip_encode_2(sent, ngram, window_size, args=None, tokenizer=None, model=None, dropoff=None):
    ...
    for i in range(iters):
        ...
        pre_embs.append(pre_emb.detach().cpu())
        post_embs.append(post_emb.detach().cpu())
        ...
        if args.resume_fast:
            ...
            if not args.multi_index:
                ...
                if os.path.exists(os.path.join(args.data_folder, original_name, "test_prefix_embeddings.pt")):
                    copyfile(os.path.join(args.data_folder, original_name, "test_prefix_embeddings.pt"), os.path.join(args.out_folder, folder_name, "test_embedding_mappings.pkl"))
