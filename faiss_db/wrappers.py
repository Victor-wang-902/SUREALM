...
class RetrievalGenerationModel(BertLMHeadModel):
    def __init__(self, config, args=None, tokenizer=None):
        super().__init__(config)
        self.bert = models.RetrievalModel(config)
        ...
        self.cross_attention = self.bert.config.add_cross_attention
    ...
    def load_embeddings(self, pre, post):
        self.bert.load_embeddings(pre,post)

    def set_retrieval(self, bool=True, pre=None, post=None):
        if bool:
            self.bert.is_retrieval(pre, post)
        else:
            self.bert.disable_retrieval()
    ...
    def post_post_init(self, args=None, from_checkpoint=None, has_cross_attention=None, init_cross_attention_weights=None, not_cross_attention=None, not_retrieval=None, pre_path=None, post_path=None, freeze_backbone=None, max_length=None, max_context_length_per_k=None): # if the model is initialized from huggingface pretrained model without cross attention, call this function after from_pretrained
        ...
        if from_checkpoint is not None and has_cross_attention is not None and init_cross_attention_weights is not None:
            if not from_checkpoint:
                if self.bert.config.add_cross_attention and not has_cross_attention: # initialize model specific parameters from original model with modified config
                    if init_cross_attention_weights:
                        self.bert.init_cross_attention_weights()
    ...
    def forward(self, 
        ...
        prediction_scores = self.cls(sentence_output)
    ...
    def save_pretrained(self, output_path, save_head=False, save_emb=False):
        ...
        if save_head:
            if not save_emb:
                try:
                    key, value = self.bert.clear_embeddings()
                except:
                    key, value = None, None
...
class RetrievalGenerator(RetrievalGenerationModel):
    def __init__(self, config, args=None, tokenizer=None):
