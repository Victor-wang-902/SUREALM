...
if __name__ == "__main__":
    ...
    args.config = AutoConfig.from_pretrained(os.path.join(args.model_path, "1"))
    ...
    args.tokenizer = AutoTokenizer.from_pretrained(os.path.join(args.model_path, "1"))
    ...
    if model_type == "bert":
        ...
        is_baseline = True
    paths = sorted(os.listdir(args.model_path))
    paths.remove("data_config.json")
    paths.remove("train_script.py")
    if "final" in paths:
        paths = paths[:-1]
        paths.sort(key=lambda x:int(x))
        paths.append("final")
    else:
        paths.sort(key=lambda x:int(x))
    ...
    for path in paths:
        if args.from_scratch:
            ...
            try:
                args.model = model_class.from_pretrained(os.path.join(args.model_path,path), config=args.config, args=args)
            except:
                args.model = model_class.from_pretrained(os.path.join(args.model_path,path), config=args.config)
        ...
        handler.evaluate()
        if args.test_eval:
            handler.evaluate(test=args.test_eval)
        logging.info("evaluate on " + os.path.join(args.model_path,path) +" done")
    #train_function(args)
    # Start producer
