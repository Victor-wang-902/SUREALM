...
from wrappers_gpt2 import RetrievalGenerationModelGPT2
from wrappers_roberta import RetrievalGenerationModelRoberta
from wrappers import RetrievalGenerationModel
from models_baselines import BertLMHeadModelBaseline
from models_baselines_roberta import RobertaForCausalLMBaseline
from models_baselines_gpt2 import GPT2LMHeadModelBaseline
import logging
...
def update_config_retrieval(args):
    ...
    logging.info(f"in update config, concat_self: {args.config.concat_self}")
    ##rerun debug
...
if __name__ == "__main__":
    ...
    parser.add_argument('--fair_baseline', action="store_true", default=False)
    parser.add_argument('--steps', type=int, default=None)
    #parser.add_argument('--save_steps', type=int, default=1000)
    parser.add_argument('--lr', type=float, default=2e-5)
    parser.add_argument('--save_epochs', type=int, default=1)
    parser.add_argument('--batch_size_per_gpu', type=int, default=64)
    parser.add_argument('--max_length', type=int, default=None) #generation max length
    ...
    if args.tokenizer.pad_token is None:  # add pad token for gpt2
        ...
        args.tokenizer.pad_token = args.tokenizer.add_special_tokens({"pad_token": "[PAD]"})
    is_baseline = False
    if args.not_retrieval:
        is_baseline = True
    if model_type == "bert":
        if args.fair_baseline:
            model_class = BertLMHeadModelBaseline
            is_baseline = True
        else:
            model_class = RetrievalGenerationModel
    elif model_type == "roberta":
        if args.fair_baseline:
            model_class = RobertaForCausalLMBaseline
            is_baseline = True
        else:
            model_class = RetrievalGenerationModelRoberta
    elif model_type == "gpt2":
        if args.fair_baseline:
            model_class = GPT2LMHeadModelBaseline
            is_baseline = True
        else:
    ...
    if not is_baseline or args.fair_baseline:
