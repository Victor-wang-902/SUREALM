...
from ..modeling import RetrievalModel
from ..utils import AttentionMaskGenerator, encode


'''
Adapted from LM models on Huggingface. We also modified the GenerationMixin to suit our need for online retrieval decoding.
'''
...
class RetrievalGenerationModel(BertLMHeadModel):
    def __init__(self, config, args=None, tokenizer=None):
        ...
        self.bert = RetrievalModel(config)
        ...
        self.is_retrieval = self.config.is_retrieval  # Modified here
        if args is not None:
    ...
    def post_post_init(
        ...
        init_cross_attention_weights=None, 
        not_cross_attention=None, 
        not_retrieval=None,
        freeze_backbone=None, 
        max_length=None, 
        max_context_length_per_k=None
        ): 
        '''
        Custom initialization AFTER model's post_init() is called. 
        One can initialize weights for "cross" attention blocks from self attention weights of a PRETRAINED model,
        disable or activate retrieval mode for a retrieval LM, and change various settings.
        If a model is initialized from huggingface without second attention blocks but is intended to be used as SUREALM, 
        this function initializes new weights if args.init_cross_attention_weights is set to True.
        If a model has been trained as SUREALM but needs to be treated as a regular LM, set args.not_retrieval to False.
        Parameters are all included in args and can be explicitly set if args is set to None.
        '''
        if args is not None:
        ...
        if not_cross_attention is not None:
            self.cross_attention = not not_cross_attention  # this does nothing for now. when retrieval is activated, the model will use the second attention block if it is there.
        if not_retrieval is not None:  # setting retrieval mode
            self.set_retrieval(not not_retrieval)
        if freeze_backbone is not None and freeze_backbone:  # whether to freeze backbone
            for param in self.bert.parameters():
                param.requires_grad = False
        if max_length is not None:  # maximum length for generation
            self.config.max_length = max_length
        if max_context_length_per_k is not None:  # maximum context length for generation, similar to that in retrieval_dataset.py
    ...
    def forward(self, 
        ...
        '''
        encoder_hidden_keys(values) are the retrieval information, will be pass to the only attention block
        if only one attention block is given.
        '''
...
class RetrievalGenerator(RetrievalGenerationModel):
    '''
    Wrapper for inference(generation) with online retrieval mechanism
    '''
    def __init__(self, config, args=None, tokenizer=None):
    ...
    def prepare_inputs_for_generation(self, input_ids, **kwargs):
        '''
        For each autoregressive iteration, we prepare input by adding retrieval information
        and adding embeddings to a cache that stores all the retrieved embeddings.
        '''  
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        '''
        Insert our custom prepare_inputs_for_generation(...) to achieve retrieval decoding.
        Most of the code here are directly copied from Huggingface transformers library.
        '''
